...
import { ArrowLeft, Save, RefreshCw, Tag, Calendar, Info, Loader2 } from "lucide-react";
...
export default function EditCouponPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const couponRes = await adminService.getCouponById(id);
        const coupon = couponRes?.data || couponRes;
        setForm({
          code: coupon.code || "",
          description: coupon.description || "",
          type: (coupon.type || "PERCENTAGE").toLowerCase(),
          value: coupon.value?.toString() || "",
          minOrder: coupon.minimumOrder?.toString() || "",
          maxDiscount: coupon.maximumDiscount?.toString() || "",
          usageLimit: coupon.usageLimit?.toString() || "",
          startDate: coupon.startsAt ? coupon.startsAt.split("T")[0] : "",
          endDate: coupon.expiresAt ? coupon.expiresAt.split("T")[0] : "",
