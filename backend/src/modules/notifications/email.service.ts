...
export class EmailService {
  ...
  private mode: 'brevo' | 'smtp' | 'mock' = 'mock';

  constructor(private readonly configService: ConfigService) {
    this.fromAddress = this.configService.get('SMTP_USER') || 'noreply@apnakit.in';

    const brevoKey = this.configService.get('BREVO_API_KEY');
    if (brevoKey && !brevoKey.startsWith('your-')) {
      this.brevoApiKey = brevoKey;
      this.mode = 'brevo';
      this.logger.log('Email service ENABLED (mode: Brevo API)');
      return;
    }
    ...
    if (hasValidCreds) {
      ...
      try {
        ...
        this.logger.log(`Email service ENABLED (mode: SMTP host: ${host})`);
      } catch (e: any) {
        this.logger.error(`Failed to create SMTP transporter: ${e.message}`);
        this.mode = 'mock';
      }
    } else {
      this.logger.log('Email service in MOCK mode (set BREVO_API_KEY or SMTP_USER/SMTP_PASS to enable)');
    }
  }
  ...
  private async sendViaBrevo(to: string, subject: string, htmlContent: string, textContent: string): Promise<{ success: boolean; message: string }> {
    if (!this.brevoApiKey) {
      return { success: false, message: 'No Brevo API key' };
    }

    const body = {
      sender: { name: 'ApnaKit', email: this.fromAddress },
      to: [{ email: to }],
      subject,
      htmlContent,
      textContent,
    ...
    try {
      ...
      if (response.ok) {
        ...
        return { success: true, message: `Brevo email sent, messageId: ${data.messageId}` };
      } else {
        const errText = await response.text();
        return { success: false, message: `Brevo API error ${response.status}: ${errText}` };
      }
    } catch (err: any) {
      clearTimeout(timeout);
      return { success: false, message: `Brevo error: ${err.message}` };
    }
  }

  private async sendViaSmtp(to: string, subject: string, html: string, text: string): Promise<{ success: boolean; message: string }> {
    if (!this.transporter) {
      return { success: false, message: 'No SMTP transporter' };
    }
    ...
    return { success: true, message: 'SMTP email sent' };
  }
  ...
  async sendOtp(email: string, code: string): Promise<{ success: boolean; message: string }> {
    const subject = `${code} is your ApnaKit verification code`;
    const html = `
    ...
    `;
    const textContent = `Your ApnaKit verification code is: ${code}\n\nThis code expires in 10 minutes.`;

    const result = await this.sendEmail(email, subject, html, textContent);
    if (result.success) {
      this.logger.log(`OTP email sent to ${email}`);
    } else {
      this.logger.error(`OTP email failed: ${result.message}`);
    }
    return result;
  }
  ...
  async sendOrderNotification(
    ...
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <p style="color: #9ca3af; font-size: 11px; text-align: center; margin-top: 16px;">&copy; ApnaKit. All rights reserved.</p>
      </div>
    `;
    ...
    const subject = `New Order #${orderData.orderNumber} - ApnaKit`;

    const result = await this.sendEmail(to, subject, html, textContent);
    if (result.success) {
      this.logger.log(`Order notification email sent to ${to} for order ${orderData.orderNumber}`);
    } else {
      this.logger.error(`Order notification email failed for ${orderData.orderNumber}: ${result.message}`);
    }
    return result;
  }
}
